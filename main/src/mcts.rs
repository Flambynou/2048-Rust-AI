...
impl MonteCarloTree2 {
    ...
    fn selection(&self, node_index: usize, rng: &mut SmallRng) -> usize {
        ...
        match &node.specific_information {
            TypeInfo::Spawn(spawn_info) => {
                if node.is_terminal || !spawn_info.block_spawns_left.is_empty() {
                    return node_index;
                }
    ...
    fn expansion(&mut self, fast: &fastgame::FastGame, node_index: usize, rng:&mut SmallRng) -> usize {
        ...
        match &node.specific_information { // Create a random child
            TypeInfo::Spawn(spawn_info) => {
                let random_index = rng.random_range(0..spawn_info.block_spawns_left.len());
                let new_spawn = spawn_info.block_spawns_left[random_index];
                ...
                new_child = Node {
                    ...
                    specific_information: TypeInfo::Move(MoveInfo {
                        actions_left: fast.get_possible_directions(&new_child_state),
            ...
            TypeInfo::Move(move_info) => {
                let random_index = rng.random_range(0..move_info.actions_left.len());
                let new_child_direction = &move_info.actions_left[random_index];
                let (new_child_state, move_score) = fast.make_move(&node.game_state, new_child_direction);
                let new_child_block_spawns = fastgame::FastGame::empty_list(&new_child_state).iter().flat_map(|&pair| [(pair, 2), (pair, 4)]).collect();
                new_child = Node {
                    game_state: new_child_state,
                    parent_index: Some(node_index),
                    visit_count: 0,
                    ...
                    specific_information: TypeInfo::Spawn(SpawnInfo {
                        ...
                        block_spawns_left: new_child_block_spawns,
                        total_value: 0.0,
                    }),
    ...
    fn get_best_direction(&mut self, fast: &fastgame::FastGame, time_limit: f32, iteration_limit: usize) -> game::Direction {
        ...
        return self.nodes.borrow()[0].children_indices.iter()
            .map(|child_index| {
                ...
                match &child.specific_information {
                    TypeInfo::Spawn(spawn_info) => {
                        (child.visit_count, spawn_info.move_made.clone())
                    },
                    _ => unreachable!(),
                }
            })
            ...
            NodeType::Move(move_node) => {
                move_node.children_indices.iter()
                    .map(|child_index| {
                        ...
                        match child {
                            NodeType::Spawn(spawn_node) => {
                                (spawn_node.visit_count, spawn_node.move_made.clone())
                            },
                            _ => unreachable!(),
                        }
                    })
